...
function solvePartTwo() {
  const path = vacumnBot.getPath();
  const routines = convertPathToRoutines(path);
  const inputString =
    routines.main + "\n" +
    routines.A + "\n" +
    routines.B + "\n" +
  ...
  const inputGen = utilities.generateFromArray(inputString.split("").map(x => x.charCodeAt(0)));
  intcode[0] = 2;
  utilities.computeIntcode(
    [...intcode],
    () => inputGen.next().value,
    x => x > 255 && console.log(x)
  );
}

/**
 * Converts a path to an object containing routines
 * @param {Array} path - An array containg a path, where each element is either "R", "L" or a number
 * @return {number} - An object containing the main routine and the A,B,C subroutines 
 */
function convertPathToRoutines(path) {

    for (
      let subRoutineALength = 10;
      subRoutineALength > 0;
      subRoutineALength--
    ) {
      const subRoutineA = path.slice(0, subRoutineALength).join(",");
      if (subRoutineA.length > 20) continue;

      // replace the matching parts of the path with "A"
      const routineMixedWithA = replacePartsOfRoutine(
        path,
        subRoutineALength,
        subRoutineA,
        "A"
      ...
      for (
        let subRoutineBLength = 10;
        subRoutineBLength > 0;
        subRoutineBLength--
      ) {
        const startOfB = utilities.getSmallestIndexThat(
          routineMixedWithA,
          (x) => x !== "A"
        ); // subroutine B starts as soon as A is not used
        const subRoutineB = routineMixedWithA
          .slice(startOfB, startOfB + subRoutineBLength)
          .join(",");
        if (subRoutineB.length > 20 || subRoutineB.indexOf("A") !== -1)
          continue;

        const routineMixedWithAB = replacePartsOfRoutine(routineMixedWithA, subRoutineBLength, subRoutineB, "B");

        const startOfC = utilities.getSmallestIndexThat(
          routineMixedWithAB,
          (x) => x !== "A" && x !== "B"
        );
        const endOfC = utilities.getSmallestIndexThat(
          routineMixedWithAB,
          (x) => x === "A" || x === "B",
          startOfC + 1
        );
        const subRoutineC = routineMixedWithAB
          .slice(startOfC, endOfC)
          .join(",");
          
        const subRoutineCLength = endOfC - startOfC;
        const mainRoutine = replacePartsOfRoutine(routineMixedWithAB, subRoutineCLength, subRoutineC, "C");

        const found =
          utilities.getSmallestIndexThat(
            mainRoutine,
            (x) => ["A", "B", "C"].indexOf(x) == -1
          ) === undefined;
...
function replacePartsOfRoutine(routine, lengthToMatch, match, newValue) {
  const newRoutine = [...routine];
  for (let i = newRoutine.length - lengthToMatch; i >= 0; i--) {
    if (newRoutine.slice(i, lengthToMatch + i).join(",") === match) {
      newRoutine.splice(i, lengthToMatch, newValue);
    }
  }
  return newRoutine;
}