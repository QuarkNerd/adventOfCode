...
fn get_input() -> Vec<PuzzleUnit<'static>>  {
    let list = INPUT.split('\n').map(|x| x.split(' ').collect::<Vec<&str>>());
    let list = list.map(|x| {
        let limits = x[0].split('-').collect::<Vec<&str>>();    
        
        PuzzleUnit {
            password: x[2],
            letter: x[1].chars().next().unwrap(),
            min: limits[0].parse::<usize>().unwrap(),
            max: limits[1].parse::<usize>().unwrap(),
        }
    });
    list.collect()
}

static INPUT: &str = "16-18 h: hhhhhhhhhhhhhhhhhh
