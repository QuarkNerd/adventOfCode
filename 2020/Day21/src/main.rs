...
lazy_static::lazy_static! {
    static ref INGREDIENT_REGEX: Regex = Regex::new(r"(?P<ingredients>[a-z ]+) \(contains (?P<allergens>[a-z ,]+)\)").unwrap();
}

fn main() {
    let mut allergens_to_pos_ingredients: HashMap<&str,HashSet<&str>> = HashMap::new();
    let food_items = get_input();

    for (ingredients, allergens) in &food_items {
        for aller in allergens {
            if let Some(ingredient_hash) = allergens_to_pos_ingredients.get_mut(aller) {
                *ingredient_hash = ingredient_hash.intersection(&ingredients).map(|&x| x).collect();
            } else {   
                allergens_to_pos_ingredients.insert(aller, ingredients.clone());
            }
        }
    }
    let mut potentially_dangerous_ingredients = HashSet::new();
    for (_, ingredients) in &allergens_to_pos_ingredients {
        potentially_dangerous_ingredients = potentially_dangerous_ingredients.union(&ingredients).map(|&x| x).collect::<HashSet<&str>>();
    }



    let mut count = 0;
    for (ingredients, _) in &food_items {
        count += ingredients.difference(&potentially_dangerous_ingredients).count();
    }
    println!("Part one: {}", count);
    
    let mut already_identified_fields = HashSet::new();
    for _ in 0..allergens_to_pos_ingredients.len() {
        let mut current_ingredient: &str = "";
        
        for (_, ingredients) in &allergens_to_pos_ingredients {
            if ingredients.len() == 1 {
                current_ingredient = ingredients.iter().next().unwrap();
                
                if already_identified_fields.get(&current_ingredient).is_none() {
                    already_identified_fields.insert(current_ingredient);
                    break;
                }
            }
        }
        
        for (_, ingredients) in &mut allergens_to_pos_ingredients {
            if ingredients.len() != 1 {
                ingredients.remove(&current_ingredient); 
            }
        }
        
    };

    let mut canonical_dangerous_ingredient_list = allergens_to_pos_ingredients.iter().collect::<Vec<_>>();
    canonical_dangerous_ingredient_list.sort_by(|(allergen_a,_), (allergen_b,_)| allergen_a.partial_cmp(allergen_b).unwrap());
    let canonical_dangerous_ingredient_list = canonical_dangerous_ingredient_list.iter().map(|(_, ingred)| *ingred.iter().next().unwrap())
                                              .collect::<Vec<_>>().join(",");

    println!("Part two: {}", canonical_dangerous_ingredient_list);
}

// Why does this work if we defeine INPUT insode func
fn get_input<'a>() -> Vec<(HashSet<&'a str>, Vec<&'a str>)>{
    INPUT.split('\n').map(|x| {
        parse_input_line(x)
    }).collect()
}

fn parse_input_line<'a>(line: &'a str) -> (HashSet<&'a str>, Vec<&'a str>) {
     let captures = INGREDIENT_REGEX.captures(line).unwrap();
     let ingredients = captures.name("ingredients").unwrap().as_str().split(' ').collect();            
     let allergens = captures.name("allergens").unwrap().as_str().split(", ").collect();            
