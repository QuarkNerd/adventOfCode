...
lazy_static::lazy_static! {
    static ref HAIR_COLOUR_REGEX : Regex = Regex::new(r"^#[\da-f]{6}$").unwrap();
    static ref HEIGHT_CAPTURE_REGEX : Regex = Regex::new(r"^(\d{2,3})(cm|in)$").unwrap();
...
fn main() {
    ...
    let passports: Vec<HashMap<&str, &str>> =  passports.into_iter()
          .filter(|pass| has_correct_fields(pass))
          .collect();
        
    println!("Part one: {}", passports.len());
    let passports: Vec<HashMap<&str, &str>> =  passports.into_iter()
          .filter(|pass| are_fields_accurate(pass))
          .collect();

    println!("Part two: {}", passports.len());
}

fn has_correct_fields(pass: &HashMap<&str, &str>) -> bool {
    pass.len() == 8 || (pass.len() == 7 && pass.get("cid").is_none())
}

fn are_fields_accurate(pass: &HashMap<&str, &str>) -> bool {
        for (field, value) in pass {
                let is_valid = match *field {
                    "byr" => is_num_between(value, 1920, 2002),
                    "iyr" => is_num_between(value, 2010, 2020),
                    "eyr" => is_num_between(value, 2020, 2030),
                    "pid" => PID_REGEX.is_match(value),
                    "ecl" => ["amb", "blu", "brn", "gry", "grn", "hzl", "oth"].iter().any(|i| i==value),
                    "hcl" => HAIR_COLOUR_REGEX.is_match(value),
                    "hgt" => {
                        let caps = HEIGHT_CAPTURE_REGEX.captures(value);
                        if caps.is_none() {
                            return false
                        }; 
                        ...
                        match caps.get(2) {
                            Some(units) => {
                                match units.as_str() {
                                    "cm" => is_num_between(caps.get(1).unwrap().as_str(), 150, 193),
                                    "in" => is_num_between(caps.get(1).unwrap().as_str(), 59, 76),
                                    other => panic!("invalid height units: {}", other)
                                }
                            }
                            None => panic!("None")
                        }
                    ...
                    "cid" => true,
                    other => panic!("invalid passport field: {}", other), 
                };
...
fn is_num_between(value: &str, min: u32, max: u32) -> bool {
...
fn get_input() -> Vec<HashMap<&'static str, &'static str>>{
    let separator = Regex::new(r"[\n\s]").expect("Invalid regex");

    INPUT.split("\n\n").map(|x| {
        let mut fields_hash = HashMap::new();
        for field in separator.split(x).into_iter() {
            let mut field = field.split(":");
            fields_hash.insert(field.next().unwrap(), field.next().unwrap());
        }
        fields_hash
    }).collect()
}

static INPUT: &str = "eyr:2021 hgt:168cm hcl:#fffffd pid:180778832 byr:1923 ecl:amb iyr:2019 cid:241
