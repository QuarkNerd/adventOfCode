    ...
    for group in input {
        ...
        for character in group.chars() {
            if character == '\n' {
                ...
                *hash.entry(character).or_insert(0) += 1;
            }
        }
        part_one_counter += hash.len();
        let stuff: Vec<_> = hash.iter().filter(|&(_, counter)| counter == &lines).collect();
        part_two_counter += stuff.len();
    }
