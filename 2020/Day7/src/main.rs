...
fn main() {
    let OUTER_BAG_REGEX : Regex = Regex::new(r"([a-z]+\s?[a-z]+) bags contain").unwrap();
    let INNER_BAG_REGEX = Regex::new(r"(\d+) ([a-z]+\s?[a-z]+) bags?").unwrap();
    let mut outer_to_inner_bag_hash = HashMap::new();
    let mut inner_to_outer_bag_hash = HashMap::new();
    let rules_list = INPUT.split(".\n");

    for rule in rules_list {
        let outer_bag_regex_match = OUTER_BAG_REGEX.captures(rule).unwrap();
        let inner_bags_list = INNER_BAG_REGEX.captures_iter(rule).map(|x| {
            let a = x[1].parse::<u16>().unwrap();
            (a, x[2].to_string())
        });

        for inner_bag in inner_bags_list {
            inner_to_outer_bag_hash.entry(inner_bag.1.clone()).or_insert(Vec::<String>::new()).push(outer_bag_regex_match[1].to_string());
            outer_to_inner_bag_hash.entry(outer_bag_regex_match[1].to_string()).or_insert(Vec::<(u16, String)>::new()).push(inner_bag);
        }
    }


    let mut part_one_bag_set = HashSet::new();
    add_bags(&mut part_one_bag_set, &inner_to_outer_bag_hash, "shiny gold");
    let result = count_bags(&outer_to_inner_bag_hash, "shiny gold");

    println!("Part one: {}", part_one_bag_set.len() - 1);
    println!("Part two: {}", result);
}

fn count_bags(rules: &HashMap<String, Vec<(u16,String)>>, bag_to_add: &str ) -> u16 {
    let mut count = 0;
    
    if let Some(bags_array) = rules.get(bag_to_add) {
        for (quantity, bag) in bags_array {
            count += quantity;
            count += quantity*count_bags(rules, bag)
        }
    }
...
fn add_bags<'a>(bag_set: &mut HashSet<&'a str>, rules: &'a HashMap<String, Vec<String>>, bag_to_add: &'a str) {
    bag_set.insert(bag_to_add);

    if let Some(bags_array) = rules.get(bag_to_add.clone()) {
        for bag in bags_array {
            add_bags(bag_set,  rules, bag);
        }
    }
}

static INPUT: &str = "wavy turquoise bags contain no other bags.
