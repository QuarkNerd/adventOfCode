...
function getThrusterOutputInNormalMode (intcode, phaseOrder) {
  ...
  for (let i = 0; i < phaseOrder.length; i++) {
    result = computeAmp([...intcode], phaseOrder[i], input)
    input = result.value
  }
  return input
}
...
function getThrusterOutputInFeedbackMode (intcode, phaseOrder) {
  ...
  while (true) {
    ...
    result = computeAmp(intcodesList[i % 5], phaseOrder[i % 5], input, i > 4)
    if (result.halted) {
      break
    } else {
      i++
...
function computeAmp (C, phaseSetting, inputFromPrev, phaseGiven = false) {
  let i = 0
  let opcode
  let instruction
