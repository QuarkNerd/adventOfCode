...
const solver = (A, B) => {
  let closestCrossing = Number.MAX_VALUE;
  let passedPoints = {};
  let current = { x: 0, y: 0 };
  for (j in A) {
    let order = A[j];
    ...
    if (direction === "U") {
      for (i = current.y + 1; i <= current.y + distance; i++) {
        passedPoints[`${current.x}_${i}`] = true;
      }
      ...
      for (i = current.x + 1; i <= current.x + distance; i++) {
        passedPoints[`${i}_${current.y}`] = true;
      }
  ...
  for (j in B) {
    ...
    if (direction === "U") {
      for (i = current.y + 1; i <= current.y + distance; i++) {
        if (
          passedPoints[`${current.x}_${i}`] &&
          closestCrossing > Math.abs(i) + Math.abs(current.x)
        ) {
          closestCrossing = Math.abs(i) + Math.abs(current.x);
        }
      }
      ...
      for (i = current.x + 1; i <= current.x + distance; i++) {
        if (
          passedPoints[`${i}_${current.y}`] &&
          closestCrossing > Math.abs(i) + Math.abs(current.y)
        ) {
          closestCrossing = Math.abs(i) + Math.abs(current.y);
        }
      }
